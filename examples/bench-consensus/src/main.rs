//! Consensus throughput benchmark using real multi-process cluster.
...
fn run_bench(label: &str, binary: &std::path::Path) {
    let base_dir = std::env::temp_dir().join(format!(
        "hotmint-bench-consensus-{}-{}",
        std::process::id(),
        label.replace(' ', "-")
    ));
    ...
    // Start nodes with staggered startup.
    ...
    // Wait a moment for blocks to accumulate.
    std::thread::sleep(Duration::from_secs(2));
    let start_height = query_height(hotmint_mgmt::loopback_addr(), rpc_port).unwrap_or(0);
    let start = Instant::now();

    std::thread::sleep(Duration::from_secs(DURATION_SECS));

    let elapsed = start.elapsed();
    let end_height = query_height(hotmint_mgmt::loopback_addr(), rpc_port).unwrap_or(start_height);
    ...
    println!(
        "    Result: {blocks_per_sec:.1} blocks/sec, {ms_per_block:.1} ms/block, {blocks} blocks (height {start_height}→{end_height})"
    );
    println!();
