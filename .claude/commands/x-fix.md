...
2. **Implement** a complete fix. The fix must:
   ...
   - Respect BFT consensus invariants: quorum correctness, view-change safety, leader-election fairness
...
3. Execute the `/x-review` Execution Protocol on the diff — including BFT-specific invariant checks, consensus safety analysis, and unsafe block audit.
...
4. Bump patch version in workspace `Cargo.toml` (`[workspace.package]`) if any `.rs` files changed (see `/x-commit` Task 4).
